...
# MODEL_PATH = "best.pt"
# # MODEL_PATH = r"C:\Users\WORK-HOME\Documents\GitHub\ForestGuard-main\backend\best.pt"
# UPLOAD_FOLDER = "uploads"
# OUTPUT_DIR = "output"
# CHM_OUTPUT = os.path.join(OUTPUT_DIR, "chm.npy")
# HIGH_RISK_OUTPUT = os.path.join(OUTPUT_DIR, "high_risk_locations.json")
...
# Remove references to best.pt file
UPLOAD_FOLDER = "uploads"
OUTPUT_DIR = "output"
CHM_OUTPUT = os.path.join(OUTPUT_DIR, "chm.npy")
HIGH_RISK_OUTPUT = os.path.join(OUTPUT_DIR, "high_risk_locations.json")
...
# Download a pre-trained YOLOv5 model from PyTorch Hub
# Using YOLOv5s, which is smaller but should work for this purpose
try:
    ...
    try:
        print("Attempting to load YOLOv5n (nano) model...")
        model = torch.hub.load('ultralytics/yolov5', 'yolov5n', pretrained=True)
        print("Fallback model loaded successfully!")
    except Exception as e2:
        print(f"Error loading fallback model: {e2}")
        model = None
        print("WARNING: No model loaded. Detection functions will not work.")
...
def detect_powerlines():
    try:
        if model is None:
            return jsonify({"error": "No model loaded. Please restart the application."}), 500
        ...
        results = []
        for image_path in image_paths:
            # Using YOLOv5 model to detect objects
            result = model(image_path)
            
            # Filter for poles and power lines (classes may vary based on model)
            # Standard YOLOv5 doesn't have specific powerline classes, so this is a placeholder
            # You might need to customize this based on your requirements
            
            # Save results
            result.save(save_dir=os.path.join(OUTPUT_DIR, "results"))
            results.append(os.path.basename(image_path))

        return jsonify({
            "message": f"Detection complete for {len(results)} images.",
...
def generate_chm():
    try:
        ...
        print(f"Reading LAS file: {filename}")
        las = laspy.read(filename)
        x, y, z = las.x, las.y, las.z
        classification = las.classification
        return_number = las.return_number
        
        print("Generating CHM grid...")
        chm_grid, xmin, ymax, resolution = generate_chm_grid(x, y, z, classification, return_number)
        np.save(CHM_OUTPUT, chm_grid)
        
        print("Finding high risk trees...")
        high_risk_trees = find_high_risk_trees(chm_grid, xmin, ymax, resolution)
        save_high_risk_locations(high_risk_trees, HIGH_RISK_OUTPUT)
        ...
        print(f"Error in generate-chm: {e}")
        return jsonify({"error": str(e)}), 500
...
# Update paths to use os.path.join for cross-platform compatibility
UPLOAD_FOLDER = "uploads"
OUTPUT_DIR = "output"
CHM_OUTPUT = os.path.join(OUTPUT_DIR, "chm.npy")
HIGH_RISK_OUTPUT = os.path.join(OUTPUT_DIR, "high_risk_locations.json")

# Use relative paths when possible, absolute paths when necessary
FIRE_SPREAD_MODEL = os.path.join(os.getcwd(), "fire_spread_predictor.pkl")
...
# Load fire spread model if it exists
try:
    fire_spread_model = joblib.load(FIRE_SPREAD_MODEL)
    scaler = joblib.load(SCALER_PATH)
    print("Fire spread model and scaler loaded successfully")
except Exception as e:
    print(f"Error loading fire spread model: {e}")
    fire_spread_model = None
    scaler = None

@app.route('/predict', methods=['POST'])
def predict_fire_risk():
    try:
        if fire_spread_model is None or scaler is None:
            return jsonify({"error": "Fire spread model not loaded. Check paths and try again."}), 500
